...
void Gate_ConstrInputsOutputs_OutputIDShouldMatch() {
    Wire w0, w1;
    Gate g0(AND, {&w0}, {&w1});
    BGL_UTL.AssertEqual(g0.GetOutputs().at(0)->GetID(), w1.GetID());
}

void Gate_ConstrInputsOutputs_OutputsShouldBeEmpty() {
    Gate g0(AND, {}, {});
    BGL_UTL.AssertTrue(g0.GetOutputs().empty());
}
...
void Gate_GetOutputs_IDShouldMatch() {
    ...
    BGL_UTL.AssertEqual(g0.GetOutputs().at(0)->GetID(), w4.GetID());
}
...
void None_ConnectGateWire_GateOutputIDShouldMatch() {
    ...
    Gate g0;
    Connect(&g0, &w0);
    BGL_UTL.AssertEqual(g0.GetOutputs().at(0)->GetID(), w0.GetID());
}
